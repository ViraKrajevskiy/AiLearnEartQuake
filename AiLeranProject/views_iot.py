...
from .geoip_utils import get_client_ip, get_country_by_ip  # ✅ ИМПОРТ
...
def iot_latest_data(request):
    ...
    return Response({
        ...
        "country": latest.country if latest else "N/A"
    })
...
def iot_sensor_endpoint(request):
    ...
    Endpoint для приема данных с ESP32 (только вибрация и статус)
     ОБНОВЛЕНО: Определяет страну по IP и сохраняет depth
    """
    try:
        data = request.data if isinstance(request.data, dict) else json.loads(request.body)

        #  ПОЛУЧАЕМ IP И ОПРЕДЕЛЯЕМ СТРАНУ
        client_ip = get_client_ip(request)
        geo_info = get_country_by_ip(client_ip)
        ...
        #  ПОЛУЧАЕМ DEPTH ИЗ ЗАПРОСА, ИНАЧЕ DEFAULT
        try:
        ...
        response_data = {
            ...
            'depth': depth,  #  ВОЗВРАЩАЕМ DEPTH
            'country': geo_info.get('country') if geo_info else 'Unknown',  # ✅ ВОЗВРАЩАЕМ СТРАНУ
        }
...
def iot_sensor_stats(request, sensor_id=None):
    """
    Статистика по вибрации, заряду батареи и СТРАНЕ
    """
    ...
    stats = {
        ...
        'countries': countries,  #  ДОБАВЛЕНО
        'depth_info': {  #  НОВОЕ
            'min': min([d.depth for d in qs if d.depth]) if any(d.depth for d in qs) else 0,
            'max': max([d.depth for d in qs if d.depth]) if any(d.depth for d in qs) else 0,
            'avg': sum([d.depth or 0 for d in qs]) / len(qs) if qs else 0,
        }
    }
